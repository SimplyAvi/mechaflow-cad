...
def test_project_store_rejects_duplicate_artifact_ids_across_projects() -> None:
    ...
    metadata_clone.id = "project-metadata-clone"
    ...
    downloadable_sample.analysis_jobs[0].artifacts[0].payload = {
        "file_manifest": [{
            "name": "metadata.json",
            "download_url": "/api/analysis-artifacts/artifact-cad-metadata/metadata.json",
        }],
    }
    downloadable_clone = downloadable_sample.model_copy(deep=True)
