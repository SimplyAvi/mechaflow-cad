...
6. In Design mode, collapse or explode the robot arm, orbit with yaw and pitch, shift-drag to pan, zoom, then select a concrete arm part directly in the canvas or from the Project drawer model tree. The canvas annotations, synchronized left inspector, viewport-anchored editor, and contextual Review drawer selected-part card should agree on the same actual part. The model canvas should show size, constraints/features, hole placement, fastener fit, material/process, assembly relation, drawing/export evidence, definition state, value provenance, downstream impacts, and load/FEA context while the viewport editor shows editable dimensions, sketch plane/profile/extrude/cut controls, centered fastener hole placement, material/process controls, machinist drawing preview, and FEA-input preview without requiring a page scroll away from the model.
...
10. Use the canvas CAD tool palette or lower primitive palette to create a beam, joint, bracket, motor, connector, electronics block, or tool plate. Rename it with the label field, edit dimensions and XYZ position, rotate it, pick a sketch plane/profile, set an extrude or cut operation, add revolve/chamfer/fillet metadata when relevant, place a centered hole 2 in from the bottom using a selected screw/bolt size, connect it to a parent part, choose a joint type, and route a visible wire to another part. The canvas, synchronized inspector, selected-part card, model tree, visible joint marker, drawing preview, FEA-input preview, and visible wire list should update immediately.
...
19. Use the `Portable project file` card in Reports mode or the Design mode export button to export the current project as a `.mfcad.json` file. Confirm the browser export includes the selected part drawing, lifecycle evidence, downstream impact preview, provenance labels, and FEA-input preview under the visual-authoring extension.
