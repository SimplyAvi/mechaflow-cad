...
The initial UI is useful before FreeCAD, KiCad, FEA, or supplier workers exist. It now opens as a full-canvas visual CAD authoring cockpit: the SVG-based XYZ workspace dominates the page, a full-canvas action bar opens contextual Project, Part, Tools, Load, Drawing/FEA, Assembly, Command, or Review pop-outs, clickable on-model annotations show selected size, constraints, features, holes, fastener fit, material/process, assembly relations, drawing/export evidence, and load/FEA context directly on the working plane, and synchronized left plus right selected-part editors stay inside the model frame. Users can choose working units, start from a blank part-design plane or a loaded assembly, create editable visual primitives, adjust selected-part dimensions without leaving the 3D profile, define sketch plane/profile/extrude/cut/revolve/chamfer/fillet metadata, place fastener-aware holes, assemble robot or machine links, place motors and connectors, route visible wiring, preview part drawing, lifecycle evidence, and FEA-input packets, and save the authored model as `.mfcad.json`. Advanced analysis, manufacturing, reporting, and backend handoff panels are grouped behind mode buttons instead of appearing as a startup checklist.

It includes mocked orchestration data for:

- Opening a bundled robot arm visual MVP with a wrist gripper.
- Viewing an interactive visual CAD workspace with XYZ grid lines, axis labels, orbit, pan, zoom, yaw, pitch, and explode controls.
- Selecting actual robot-arm parts directly on the canvas or from the assembly tree with unmistakable canvas, model-tree, selected-part-card, and inspector highlighting.
...
- Editing selected geometry dimensions, sketch definition state, constraints, features, holes, material/process, mates, payload, and drawing handoff from clickable on-model annotations mirrored in the synchronized left inspector and viewport-anchored inspector next to the 3D profile.
- Picking visual CAD operation state for the selected part: plane, 2D profile, constraints, extrude depth, cut/drill operation, and finish step metadata.
- Placing fastener-aware holes such as a centered hole 2 in from the bottom, with clearance diameter defaulted from local M4, M5, M6, M8, or 1/4-20 screw choices.
- Previewing build-ready individual part outputs: machinist drawing metadata with units, dimensions, hole placement, material, process, and fastener callouts, plus lifecycle evidence with definition state, value provenance, downstream impacts, reviewable fixes, and a structured FEA-input preview with dimensions, material, load requirements, self-weight, constraints, and review-required notes.
- Showing a CAD lifecycle map that connects public CAD concepts such as sketches, relations, features, configurations, hardware, assemblies, drawings, BOM, export, and simulation setup to MechaFlow project data, API handoff fields, and review-required worker boundaries.
...
- Resizing the robot-arm requirement from 50 lb to 75 lb with deterministic local load triage that accounts for payload plus assembly self-weight, safety factor, reach, actuator torque, fasteners, hinges, sleeves or couplers, brackets, arm links, and end-effector parts. Catalog-backed fixes update visible geometry, fastener callouts, material or actuator metadata, while remaining review-required and not FEA.
...
- Showing a prominent selected-part detail card near the canvas with part name, purpose, primitive role, dimensions, material and process rationale, weight or review-required mass, cost or review-required cost, stress or capability state, source confidence, task thresholds, load cases, constraints, wiring or serviceability criteria, demo-estimate labels, and review-required warnings.
...
- Exporting and importing portable `.mfcad.json` project files directly in the browser or through a connected FastAPI backend or desktop mock API.
...
1. Launch the app and start in the visual CAD workspace. Orbit, pan, zoom, explode, or click a concrete part before opening any advanced panel.
2. Confirm selection is visible in the canvas and viewport-anchored editor, then open the Project or Review contextual drawer to inspect the model tree, selected-part detail card, and criteria. Use keyboard focus plus Enter or Space to select a canvas part accessibly.
...
4. Create parts from the canvas CAD tool palette or primitive palette, then click canvas annotations or use the viewport inspector to edit length, width, height or diameter, material, process, sketch plane/profile, extrude or cut state, revolve/chamfer/fillet metadata, and fastener hole placement without scrolling away from the 3D model.
5. Review the on-model annotations plus the synchronized left inspector, viewport drawing preview, FEA input preview, and CAD lifecycle map for the selected part. Confirm units, dimensions, definition state, provenance, downstream impacts, material/process, hole placement, screw/bolt specs, load requirement, self-weight, constraints, project-file persistence, and review-required notes are all visible.
6. Place motors, connector blocks, electronics, and tool plates, then route visible harness polylines between parts. These routes persist as connector, wire segment, route, and BOM data but remain review-required for real electrical and CAD validation.
7. Toggle Focus selected to inspect one part in isolation, then clear focus to restore the full assembly context.
8. Use Requirement sizing triage to review payload plus assembly self-weight, safety factor, reach, effective shoulder torque, undersized component highlights, and deterministic local upgrade suggestions. The 50 lb to 75 lb demo path can apply catalog-backed fixes for actuator class, fastener size or material, sleeve or coupler thickness, arm-link geometry, bracket material, and end-effector hardware without claiming certification.
9. Type an unknown part description into the local catalog matcher. Review ranked local matches and their confidence, dimensions, material, process, and caveats, then apply one to the selected part or add it to the active assembly. The result remains editable and review-required for engineering release.
10. Type into `Describe what you want to design...`. The command line extracts chips for payload, reach, cycle time, material cues, constraints, and restrictions when it can. Click `Start design` to create a local prompt concept with units and reach prefilled on the grid. The current workspace remains a visual authoring proxy until real CAD generation exists.
11. Add reference photos or images with upload or drag/drop. Images are treated as local reference metadata only in this slice. The UI does not claim photo-to-CAD reconstruction, FEA, supplier quotes, or electrical validation.
12. Use the left sidebar to open an existing `.mfcad.json` project through the offline client path, local desktop mock, or backend, reopen the recent project, or load one of the repository-local ready examples.
13. Stay in Design mode for model tree, part selection, and authoring tools. Switch to Analysis for solver readiness and job queue, Manufacturing for BOM, make or buy paths, wiring and electronics, Reports for advisory reports and project import/export, and Backend for API handoff details.
14. Ready examples in this slice are repository-local MIT seed data. No external CAD assets were imported, so no third-party attribution is required beyond the local seed notes.
