...
The initial UI is useful before FreeCAD, KiCad, FEA, or supplier workers exist. It now opens as a full-canvas visual CAD authoring cockpit: the SVG-based XYZ workspace dominates the page, a full-canvas action bar opens contextual Project, Part, Tools, Load, Drawing/FEA, Assembly, Command, or Review pop-outs, on-model annotations show selected size, constraints, holes, fastener fit, material/process, and load/FEA context directly on the working plane, and the selected-part editor stays anchored inside the model frame. Users can choose working units, start from a blank part-design plane or a loaded assembly, create editable visual primitives, adjust selected-part dimensions without leaving the 3D profile, define sketch plane/profile/extrude/cut state, place fastener-aware holes, assemble robot or machine links, place motors and connectors, route visible wiring, preview part drawing and FEA-input packets, and save the authored model as `.mfcad.json`. Advanced analysis, manufacturing, reporting, and backend handoff panels are grouped behind mode buttons instead of appearing as a startup checklist.
...
- Editing selected geometry dimensions from a viewport-anchored inspector next to the 3D profile, while selected size, constraint, feature, hole, fastener, material/process, and load/FEA callouts stay rendered on the model.
...
- Previewing build-ready individual part outputs: machinist drawing metadata with units, dimensions, hole placement, material, process, and fastener callouts, plus a structured FEA-input preview with dimensions, material, load requirements, self-weight, constraints, and review-required notes.
...
4. Create parts from the canvas CAD tool palette or primitive palette, then use the viewport inspector to edit length, width, height or diameter, material, process, sketch plane/profile, extrude or cut state, and fastener hole placement without scrolling away from the 3D model.
5. Review the on-model annotations plus the viewport drawing preview, FEA input preview, and CAD lifecycle map for the selected part. Confirm units, dimensions, material/process, hole placement, screw/bolt specs, load requirement, self-weight, constraints, project-file persistence, and review-required notes are all visible.
