...
The full-canvas frontend shell keeps the 3D modeling plane as the dominant workspace. Users can start from a blank part-design plane or open an existing assembly, then reveal Project, Part, Tools, Load, Drawing/FEA, Assembly, Command, or Review controls as contextual pop-outs instead of reading every panel at once. Clickable on-model annotations keep selected-part size, constraints/features, hole placement, fastener fit, material/process, assembly relation, drawing/export evidence, and load/FEA context visible directly in the working plane while the synchronized left inspector and side drawers support precise editing. The UI exposes under-defined, fully-defined, over-defined, provisional, and requirements-incomplete states, shows user-defined, inferred, defaulted, estimated, unresolved, or invalid value provenance, and previews downstream geometry, mates, hardware, drawings, BOM, manufacturing, and analysis impacts before broad fixes. The selected-part viewport also shows a CAD lifecycle map that ties public CAD process concepts to MechaFlow data persistence, API handoff fields, and future worker boundaries without claiming proprietary compatibility.
