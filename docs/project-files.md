...
The project payload preserves the meaningful MVP data: authoring units, project metadata, visual-authoring metadata, assemblies, parts, dimensions, optional primitive diameter, viewport sketch state, hole placement, fastener selections, materials, electronics components, wire segments, wiring rule sets, wiring routes, active task, modifications, analysis jobs, job recommendations, cached report references, cached artifact references, reports, and analysis job artifacts. Local catalog applications also persist the matched item, query, score, confidence, matched terms, reasoning, source, role criteria, and editable assembly-placement metadata on the part. Requirement sizing upgrades persist review-required local upgrade metadata, changed dimensions, fastener callouts, material or actuator metadata, and the payload plus self-weight assumptions used for the deterministic triage. Browser exports also include `extensions.visual_authoring_mvp.part_outputs`, a deterministic set of individual part drawing, FEA-input, and lifecycle evidence previews for review, plus `extensions.visual_authoring_mvp.cad_lifecycle_map`, the public CAD process to MechaFlow data and worker-boundary mapping. BOM, manufacturing option groupings, wiring/electronics panels, route review evidence, analysis readiness panels, and enriched job queue panels are rebuilt from the imported project through `ProjectPanelData`.

Visual authoring data is stored on each part at `metadata.visual_authoring`. The browser workspace writes fields such as `primitive`, `position_mm`, `rotation_deg`, `color`, `parent_part_id`, `joint_type`, `assigned_to_part_id`, `connector_id`, `sketch_state`, and `hole_pattern`; the in-place label editor persists its trimmed value as the part's canonical `name`. The hole pattern is mirrored to `dimensions.metadata.visual_hole_pattern` so downstream drawing and solver-prep tools can find fastener clearance, offset-from-bottom, centered-on-width, and selected screw/bolt metadata without parsing UI state. These fields drive the MVP canvas, including on-model annotations and synchronized left-inspector edits for selected dimensions, constraints/features, hole placement, fastener fit, material/process, assembly relations, lifecycle state, provenance, downstream impacts, and load/FEA context. They are intentionally review-required hints until real CAD geometry workers produce authoritative artifacts.
